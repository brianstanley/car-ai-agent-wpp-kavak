...
def main():
    ...
    agent_id = os.getenv("DEFAULT_KAVAK_AGENT_ID")
    model = os.getenv("CHAT_ASSISTANT_MODEL")
    ...
    if not instruction:
        logger.info("❌ Could not fetch memory agent data. Exiting.")
        return
    ...
    logger.info(f"👤 Usuario: {user.phone_number} (ID: {user.id})")
    logger.info(f"💬 Sesión: {chat_session_id}")
    logger.info(f"🧠 Bot ID: {agent_id}")
    ...
    while True:
        try:
            ...
            if user_input.lower() in ["/quit", "/exit", "quit", "exit"]:
                ...
                break
            if not user_input:
                logger.info("Por favor, escribe un mensaje o comando.")
                continue
            ...
            if num_tokens > MAX_USER_QUERY_TOKENS:
                ...
                logger.info(f"Tu mensaje fue muy largo y ha sido truncado a los primeros {MAX_USER_QUERY_TOKENS} tokens.")
